...
class DeploymentManager:
        ...
        try:
            ...
            try:
                ...
                if self.collection_id not in collections["CollectionIds"]:
                    logger.info(
                        f"ℹ️ Rekognition Collection '{self.collection_id}' 不存在，跳过迁移"
                    )
                    return True
            except Exception as e:
            ...
            while True:
                if next_token:
                    response = self.rekognition.list_faces(
                        CollectionId=self.collection_id, NextToken=next_token
                    )
                else:
                    response = self.rekognition.list_faces(
    ...
    def step3_deploy_lambdas(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"📊 现有Lambda函数: {len(existing_functions)}/{len(lambda_functions)}"
            )
    ...
    def step4_verify_deployment(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"📊 DynamoDB表状态: {len(existing_tables)}/{len(tables_to_check)}"
            )
            ...
            try:
                ...
                face_apis = [
                    api for api in apis["items"] if "face-recognition" in api["name"]
                ]
                if face_apis:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="OpenSearch Face Recognition 部署管理器"
    )
