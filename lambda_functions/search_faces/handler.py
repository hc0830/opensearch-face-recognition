    ...
    try:
        ...
        if not search_type or search_type not in ["by_image", "by_face_id"]:
            return create_error_response(
                400,
                'Invalid or missing search_type. Must be "by_image" or "by_face_id"',
            )
        ...
        if similarity_threshold < 0.0 or similarity_threshold > 1.0:
            return create_error_response(
                400, "similarity_threshold must be between 0.0 and 1.0"
            )
        ...
        if search_type == "by_image":
            ...
            results = search_faces_by_image(
                body["image"], collection_id, max_faces, similarity_threshold
            )
        else:  # by_face_id
            ...
            results = search_faces_by_face_id(
                body["face_id"], collection_id, max_faces, similarity_threshold
            )
...
def get_face_vector_from_opensearch(face_id: str) -> List[float]:
    ...
    try:
        ...
        # 获取文档
        response = client.get(index="face-vectors", id=face_id)
...
def search_in_opensearch(
    ...
    try:
        ...
        # 执行搜索
        response = client.search(index="face-vectors", body=search_body)
