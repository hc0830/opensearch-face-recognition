...
def index_face_with_rekognition(image_data: str, user_id: str, collection_id: str) -> Dict[str, Any]:
    """使用Rekognition索引面部"""
    try:
        # 解码base64图像
        image_bytes = base64.b64decode(image_data)
...
def store_face_metadata(face_data: Dict[str, Any], user_id: str, collection_id: str, image_key: str) -> Dict[str, Any]:
    """存储面部元数据到DynamoDB"""
    try:
...
def index_to_opensearch(face_data: Dict[str, Any], metadata: Dict[str, Any]) -> Dict[str, Any]:
    """索引面部数据到OpenSearch"""
    try:
