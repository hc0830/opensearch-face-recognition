...
def handle_migrate_collection(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 执行迁移
        results = migrate_rekognition_collection(source_collection_id, target_collection_id)
...
def migrate_rekognition_collection(
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for batch in batches:
                future = executor.submit(migrate_face_batch, batch, target_collection_id)
                futures.append(future)
...
def migrate_face_batch(faces: List[Dict[str, Any]], target_collection_id: str) -> Dict[str, Any]:
    """迁移一批面部数据"""
    migrated = 0
    failed = 0
    ...
    for face in faces:
        try:
            ...
            # 存储元数据
            store_face_metadata(face['FaceId'], face.get('UserId', 'unknown'), target_collection_id, face_doc)
...
def create_migrated_face_doc(face: Dict[str, Any], collection_id: str) -> Dict[str, Any]:
    """创建迁移的面部文档"""
    import numpy as np
...
# 重用其他Lambda函数中的辅助函数
def index_face_from_bytes(image_bytes: bytes, user_id: str, collection_id: str, s3_key: str = None) -> Dict[str, Any]:
    """索引面部（简化版本）"""
    # 这里应该调用完整的索引逻辑
    # 为了简化，返回模拟结果
...
def store_face_metadata(face_id: str, user_id: str, collection_id: str, doc: Dict[str, Any]):
    """存储面部元数据（简化版本）"""
    # 实际实现应该与index_face函数中的相同
    pass
