    ...
    try:
        ...
        if search_type == "by_image":
            ...
            matches = search_by_image(
                body["image"], max_faces, similarity_threshold, collection_id
            )
            ...
            matches = search_by_face_id(
                body["face_id"], max_faces, similarity_threshold, collection_id
            )
...
def search_by_image(
    image_data: str, max_faces: int, similarity_threshold: float, collection_id: str
) -> List[Dict[str, Any]]:
    """通过图像搜索相似面部"""
    try:
        # 解码base64图像
...
def search_by_face_id(
    face_id: str, max_faces: int, similarity_threshold: float, collection_id: str
) -> List[Dict[str, Any]]:
    """通过Face ID搜索相似面部"""
    try:
        ...
        # 首先添加查询的面部本身（100%相似度）
        try:
            face_response = rekognition.describe_collection(
                CollectionId=REKOGNITION_COLLECTION_ID
            )
...
def get_opensearch_data(face_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response = client.search(
            index="face-recognition-*", body={"query": {"term": {"face_id": face_id}}}
        )
