...
class OpenSearchFaceRecognitionStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # 创建DynamoDB表
        self.face_metadata_table = self._create_face_metadata_table(project_name, env_name)
        self.user_vectors_table = self._create_user_vectors_table(project_name, env_name)
    ...
    def _create_opensearch_domain(self, project_name: str, env_name: str) -> elasticsearch.Domain:
        """创建OpenSearch域"""
        
        # 根据环境选择实例类型
        instance_type = "t3.small.elasticsearch" if env_name == "dev" else "r6g.large.elasticsearch"
        instance_count = 1 if env_name == "dev" else 3
        
        domain = elasticsearch.Domain(
            self, "FaceRecognitionDomain",
    ...
    def _create_face_metadata_table(self, project_name: str, env_name: str) -> dynamodb.Table:
        """创建面部元数据表"""
        table = dynamodb.Table(
            self, "FaceMetadataTable",
    ...
    def _create_user_vectors_table(self, project_name: str, env_name: str) -> dynamodb.Table:
        """创建用户向量表"""
        return dynamodb.Table(
            self, "UserVectorsTable",
